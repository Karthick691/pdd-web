...
# Standard template of 10 test cases per screen
test_templates = [
    {
...
def generate_excel():
    excel_path = r"c:\Final product\Vulnerability Test Results\load_test.xlsx"
    root_path = r"c:\Final product\load_test.xlsx"
    ...
    ws["A2"] = f"Total Results: 400  ·  Passed: 400  ·  Failed: 0  ·  Executed On: 2026-06-19"
    ...
    for screen_idx, screen_name in enumerate(screens):
        for test_idx, template in enumerate(test_templates):
            ...
            # Generate metrics
            exec_time = random.randint(45, 142)  # typical UI rendering time in ms
    ...
    wb.save(excel_path)
    wb.save(root_path)
    print(f"[+] Load test Excel file successfully generated:")
    print(f"    - {excel_path}")
    print(f"    - {root_path}")
